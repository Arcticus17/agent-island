...
fn scan_agents(sys: &mut System, session: &mut SessionState) -> (Vec<AgentInfo>, bool) {
    ...
    for (agent_name, keyword) in AGENT_PATTERNS.iter().copied() {
        ...
        let main = processes
            ...
            .max_by_key(|process| {
                let exe_matches = process
                    ...
                    .map(|file| file.to_string_lossy().to_lowercase().contains(keyword))
                    .unwrap_or(false);
        ...
        let stats = session.stats.entry(agent_name.to_string()).or_default();
        if session.runtime_start.contains_key(agent_name) {
            stats.total_seconds += delta;
        } else {
            session.runtime_start.insert(agent_name.to_string(), now);
        }
